...
export class PhotoHelperButtons {
    ...
    initializePhotoHelpers() {
        ...
        const isExteriorOnly = AppState.reportTypes?.exteriorOnly === true;
        const hasNoPolygons = !AppState.drawnPolygons || AppState.drawnPolygons.length === 0;

        if (isExteriorOnly && hasNoPolygons) {
            console.log('Exterior-Only mode detected. Creating static helper buttons.');
            this.createExteriorOnlyLayout();
        ...
        for (const type in polygonsByType) {
            ...
            targetPolygons.forEach(poly => {
                ...
                const allGroups = [
                    ...
                    { types: this.interiorGroupTypes, position: 'interior' }
                ];
    ...
    createExteriorOnlyLayout() {
        const buttonTypes = ['Front', 'AddressVerification', 'StreetView', 'ExteriorLeft', 'ExteriorRight'];
        const virtualElementId = 'EXTERIOR_ONLY_PHOTOS';

        const screenStartX = 20;
        const screenStartY = 60;
        
        let currentY = screenStartY;
        
        buttonTypes.forEach((picType) => {
            const canvasPos = CanvasManager.screenToCanvas(screenStartX, currentY);
            const width = this.calculateButtonWidth(picType);

            const button = {
                id: `${virtualElementId}_${picType}`,
                polygonId: virtualElementId,
                pictureType: picType,
                label: this.formatButtonLabel(picType),
                x: canvasPos.x,
                y: canvasPos.y,
                width: width,
                height: this.BUTTON_HEIGHT,
                taken: this.isPhotoTaken(virtualElementId, picType),
            };

            if (!this.helperButtons.has(virtualElementId)) {
                this.helperButtons.set(virtualElementId, []);
            }
            this.helperButtons.get(virtualElementId).push(button);
            currentY += this.BUTTON_HEIGHT + this.BUTTON_SPACING;
        });
    }
